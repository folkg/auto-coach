...
### [x] Enable `no-async-promise-executor`
...
### [x] Enable `no-constant-binary-expression`
...
### [x] Enable `no-constant-condition`
...
### [x] Enable `no-extra-boolean-cast`
...
### [x] Enable `no-unsafe-optional-chaining`
...
### [x] Configure `curly` to `off` (from `error`)
...
### [x] Replace `await-thenable` with `@typescript-eslint/await-thenable` (error)
...
### [x] Replace `no-floating-promises` with `@typescript-eslint/no-floating-promises`
...
### [x] Replace base `require-await` with `@typescript-eslint/require-await`
...
### [x] Enable `@typescript-eslint/no-misused-promises`
...
### [x] Enable `@typescript-eslint/no-for-in-array`

- Disallow `for...in` loops over arrays.
- Fixes:
  - Replace with `for...of`, `.forEach`, or indexed `for` loops.

### [x] Enable `@typescript-eslint/no-implied-eval`
...
### [x] Enable `@typescript-eslint/no-misused-new`
...
### [x] Enable `@typescript-eslint/no-misused-spread`
...
### [x] Enable `@typescript-eslint/no-unsafe-unary-minus`

- Prevent applying unary minus to bigints or unsafe types.
- Fixes:
  - Use `-1n * value` patterns or appropriate numeric conversions.

### [x] Configure `prefer-promise-reject-errors: off` and enable `@typescript-eslint/prefer-promise-reject-errors`
...
### [x] Enable `no-unused-expressions` (from `off` to `error`)
...
### [x] Align `no-restricted-globals` with target list
...
### [x] Configure base `no-throw-literal` to `off` and enable `@typescript-eslint/only-throw-error`
...
### [x] Replace `no-explicit-any` with `@typescript-eslint/no-explicit-any`
