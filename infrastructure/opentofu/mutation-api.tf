...
# IAM binding for mutation API service account to manage Firebase Auth (token revocation, etc.)
resource "google_project_iam_member" "mutation_api_sa_firebase_auth_admin" {
  project = var.firebase_project_id
...
resource "google_cloud_run_v2_service" "mutation_api" {
  ...
  depends_on = [
    ...
    google_project_iam_member.mutation_api_sa_firebase_auth_admin,
